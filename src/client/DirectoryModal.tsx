...
export function DirectoryModal({ isOpen, onClose, state, act, onMap, hexId }: { isOpen: boolean; onClose: () => void; state: CampaignState; act: Act; onMap: (hexId: string) => void; hexId?: string }) {
  ...
  return <Modal title={`Living world directory${hexId ? ` · Hex ${hexId}` : ""}`} onClose={onClose}>
    ...
    {tab === "npcs" && canEdit && <details><summary>Record a person at the party’s current location</summary><form onSubmit={async e => { e.preventDefault(); const form = e.currentTarget; const values = new FormData(form); await act(EVENTS.NPC_RECORD, { name: values.get("name"), role: values.get("role"), ancestry: values.get("ancestry"), notes: values.get("notes") }); form.reset(); }}><label>Name<input name="name" required maxLength={80}/></label><label>Role<input name="role" required maxLength={80}/></label><label>Ancestry<input name="ancestry" required maxLength={80}/></label><label>Notes<textarea name="notes" maxLength={4000}/></label><button>Save dossier</button></form></details>}
    {tab === "npcs" && (state.worldNpcs ?? []).filter(n => (!hexId || npcHexId(n, state) === hexId) && matches(`${n.name} ${n.role} ${n.notes}`)).map(n => <article className="companion-card" key={n.id}>
      ...
      <label>Disposition<select disabled={!canEdit} value={n.disposition} onChange={e => void act(EVENTS.NPC_UPDATE_DISPOSITION, { npcId: n.id, disposition: e.target.value })}>{["uncertain", "hostile", "neutral", "friendly"].map(d => <option key={d}>{d}</option>)}</select></label>
      ...
      <label>Session notes, favors & debts<textarea disabled={!canEdit} value={drafts[n.id] ?? n.notes} onChange={e => setDrafts({ ...drafts, [n.id]: e.target.value })}/></label><button disabled={!canEdit || drafts[n.id] === undefined} onClick={async () => { await act(EVENTS.NPC_UPDATE_DISPOSITION, { npcId: n.id, disposition: n.disposition, notes: drafts[n.id] }); setDrafts(d => { const next = { ...d }; delete next[n.id]; return next; }); }}>Save notes</button>
    </article>)}
