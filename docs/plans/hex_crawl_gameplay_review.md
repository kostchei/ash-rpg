...
- Let the tavern, local encounters, and site interactions draw eligible path content through one shared insertion interface. Persist what was placed and revealed; do not reroll identities, clues, or goals on every visit. For The Night Below, retain its requirement that at least half of generated people and places remain ordinary regional content.
- Apply resolved deeds and expedition outcomes to the selected engine's own rules and saved world relationships. Do not advance every path once per travel watch, session, or arrival. Calendar-based behavior and neglect also need their authored triggers. Keep distinct path records and the permanent Toll rather than collapsing every engine into a six-step pressure clock. Reconcile the older Night Below progress helpers with the broader engine specification before choosing the runtime schema.
- Enforce path access through the same movement/site-entry service. Reuse The Night Below's aquatic-capability checks against persisted party equipment, blessings, and temporary effects. Surface clues must provide multiple discoverable preparation routes; an inaccessible underwater destination cannot become reachable merely by switching zones.
